...
if(isset($_POST['submit'])){
   ...
   if($check_cart->rowCount() > 0){
      ...
      if($address == ''){
        ...
        $insert_order = $conn->prepare("INSERT INTO `orders`(user_id, name, number, email, method, address, total_products, total_price, transaction_id) VALUES(?,?,?,?,?,?,?,?,?)");
        $insert_order->execute([$user_id, $name, $number, $email, $method, $address, $total_products, $total_price, $transaction_id]);
...
<form action="" method="post">
   ...
   <div class="cart-items">
      ...
      ?>
      <p><span class="name"><?= $fetch_cart['name']; ?></span><span class="price">ksh<?= $fetch_cart['price']; ?> x <?= $fetch_cart['quantity']; ?></span></p>
      <?php
            }
         }else{
      ...
      ?>
      <p class="grand-total"><span class="name">grand total :</span><span class="price">ksh<?= $grand_total; ?></span></p>
...
<a href="update_address.php" class="btn">update address</a>
  ...
  <select name="method" class="box" id="paymentMethod" required>
    ...
    <div id="transactionIdField" style="display: none; margin-top: 20px;">
                <label for="transaction_id">Transaction ID:</label>
                <input type="text" name="transaction_id" id="transaction_id" class="box" placeholder="Enter M-Pesa Transaction ID" required>
            </div>

            <div id="mpesaDetails" style="display: none; margin-top: 20px;">
                <p>Our Till Number: <strong>737505</strong></p>
                <p>Amount to Pay: <strong>ksh<?= $grand_total; ?></strong></p>
            </div>
